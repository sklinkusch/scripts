...
sub checkNet {
  ...
  Fahrinfo::read_finfo(\*ACPI,$station,\@arrtext);                        # subroutine to read data from ACPI and write it to @arrtext
  ...
  Fahrinfo::read_finfo(\*ACPJ,$station,\@deptext);                        # subroutine to read data from ACPJ and write it to @deptext
  close ACPJ;                                                             # close filestream ACPJ
  my $natext = $#arrtext;                                                 # define size of @arrtext
  my $ndtext = $#deptext;                                                 # define size of @deptext
  Fahrinfo::pmax_dual(\@arrtext,\@deptext);                               # cut size of @arrtext, @deptext
  Fahrinfo::cmp_dual(\@arrtext,\@deptext,\@text);                         # compose data from @arrtext and @deptext to @text
...
sub haltnummer {
  ...
  open(DATA, '/home/stefan/bin/fahrinfo-elinks2.dat') || die "can't open 'fahrinfo-elinks2.dat'"; # open filestream reading the local file fahrinfo-elinks2.dat
