...
void cp_mus(int cis_size, int nroao, int omo, int umo, int llim,  int nroe, double* cismat, double* cisvecs, double* D,
	    ...
	    ofstream *outf, ofstream *syf, int nprint, long int &nonzero, int mode);
...
int main(int argc, char* argv[]){
  ...
  syf.write((char *) &cis_size, sizeof(int));  // writing nbasis to sys file for stochastic TDCI
  syf.write((char *) &cis_size, sizeof(int));  // writing nstates to sys file for stochastic TDCI
  double* configurations = new double[cis_size];
  for(int x = 0; x < cis_size; x++) configurations[x] = 0.;
...
for(int x = 1; x < cis_size; x++){
  ...
  double* gamma = new double[cis_size];
  double* gamma_csf = new double[cis_size];
  int MO_r;
  int homo = nroe/2 - 1;
  double ip = -1.*MOens[homo];
  ...
  if(calc_d == 1){
      for(int x = 0; x < cis_size; x++) energies[x] = cisvals[x] + cisd_corr[x];
  }else{
...
//     outf << "\n";
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dx,
	 mu_core[0], MOs, Pmat, tmpmat1, cistmpmat, &outf, &syf, nprint, nonzero, 2);
...
#pragma omp parallel for
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dy,
	 mu_core[1], MOs, Pmat, tmpmat1, cistmpmat, &outf, &syf, nprint, nonzero, 2);
...
#pragma omp parallel for
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dz,
	 mu_core[2], MOs, Pmat, tmpmat1, cistmpmat, &outf, &syf, nprint, nonzero, 2);
