...
extern double   get_matrix_element(uint32_t row, uint32_t column, double* nonzero, uint32_t* col_ind, uint32_t* row_ptr); 
...
#pragma omp parallel for reduction (+:tmpijtrans)
	...
	int off_j = nroao*j;
	for(int b = 0; b < nroao; b++) tmpijtrans += i____Trans[ioff_b+b]*MOs[off_j+b];
	ij___Trans[d*nroao+c] = tmpijtrans;
      }
    }
    alt_j = j;
    //Statistics
...
#pragma omp parallel for reduction (+: tmpijktrans)
    for(int d = 0; d < nroao; d++){
      int off_k = k*nroao;
      tmpijktrans = 0.;
      ijk__Trans[d] = 0.;
      for(int c = 0; c < nroao; c++) tmpijktrans += ij___Trans[d*nroao+c]*MOs[off_k+c];
      ijk__Trans[d] = tmpijktrans;
    }
    alt_k = k;
    //Statistics
    statk++;
  ...
  double inte = 0.;
  int off_l = nroao*l;
#pragma omp parallel for reduction (+: inte)
  for(int d = 0; d < nroao; d++)
  ...
  for(int i = llim; i < llim+omo; i++){                                          // loop over all occupied orbitals
    for(int j = i; j < llim+omo+umo; j++){                                       // loop over all occupied and unoccupied orbitals with j>=i
      for(int k = llim; k < llim+omo+umo; k++){                                  // loop over all occupied and unoccupied orbitals
	   ...
	   uint32_t curr_x = calc_mo2el_ind_x(i, j, omo, umo, llim);
	   uint32_t curr_y = calc_mo2el_ind_y(k, l, omo, umo, llim); 
	   if(curr_x > x_max || curr_y > y_max){
	       *outf << "Integral (" << i << " " << j << "|" << k << " " << l << ") not included\n";
	       exit(158);
	   }
	   prec_ints[curr_x*y_max+curr_y] = mo2int_op(i, j, k, l,
...
double get_precalc_ints_sd(int i, int j, int k, int l,
  ...
  uint32_t x_max  = calc_mo2el_ind_x(omo, omo+umo-1, omo,umo, 0);
  uint32_t y_max  = calc_mo2el_ind_y(omo+umo,omo+umo, omo,  umo, 0);
  
  unsigned int x = calc_mo2el_ind_x(i, j, omo, umo, llim);
  unsigned int y = calc_mo2el_ind_y(k, l, omo, umo, llim);
  //BOUNCE CHECK
  if(x > x_max || y > y_max) { 
    cerr << "Missing integral (" << i << " " << j << "|" << k << " " << l << ")\n"; 
    exit(0);
  }
  double value = get_matrix_element((uint32_t) x, (uint32_t) y, prec_vals, prec_cols, prec_rows);
  return(value);
}
