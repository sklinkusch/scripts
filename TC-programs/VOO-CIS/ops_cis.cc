...
double get_precalc_ints_fast(int i, int j, int k, int l,
			     int omo, int umo, int llim, double* prec_ints);
double calc_cisvec(int state, int initial, int final, int omo, int umo, int llim, int cis_size, double* cisvecs);
double calc_Y(int I, int J, int a, int i, int omo, int umo, int llim, int cis_size, double* cis_vecs, double* prec_ints);
double calc_theta(int I, int J, int a, int i, int omo, int umo, int llim, int cis_size, double* MOens, double* cisens, double* cis_vecs, double* prec_ints);
...
double* precalc_ints_sd(int omo, int umo, int llim,
  ...
  *outf << "precalc_ints_sd:\n";
  ...
  // Calculate needed Memory as x_max*y_max*8
  ...
  *outf<< "Loop over O: for (Oo|ov), (Oo|vv), (Ov|ov), (Ov|vv): \n";
  ...
  for(int i = llim; i < llim+omo; i++){                                          // loop over all occupied orbitals
    for(int j = i; j < llim+omo+umo; j++){                                       // loop over all occupied and unoccupied orbitals with j>=i
      for(int k = llim; k < llim+omo+umo; k++){                                  // loop over all occupied and unoccupied orbitals
        int K = k;
        if(K < llim+omo) K = llim+omo;                                           // set K to the HOMO if K is below
        for(int l = K; l < llim+omo+umo;l++){                                    // loop over all occupied (only HOMO if included) and unoccupied orbitals 
 ...
 *                                                                             *
 * calc_Y                                                                      *
 * calculates Y_ai^IJ values for VOO-CIS correction of CIS energies            *
 *                                                                             *
 *******************************************************************************/

double calc_cisvec(int state, int initial, int final, int omo, int umo, int llim, int cis_size, double* cisvecs){
    int x = state;
    int y = (initial - llim)*umo + (final - omo - llim) + 1;
    int pos = x * cis_size + y;
    return(cisvecs[pos]);
}

double calc_Y(int I, int J, int i, int a, int omo, int umo, int llim, int cis_size, double* cisvecs, double* prec_ints){
 double first = 0.;   // first sum in equation 3, Liu et al., JCP 138, 221105 (2013)
 double second = 0.;  // second sum in equation 3, Liu et al., JCP 138, 221105 (2013)
 for(int b = llim + omo; b < (llim + omo + umo); b++){
     for(int c = llim + omo; c < (llim + omo + umo); c++){
	 for(int j = llim; j < (llim + omo); j++){
	     first += calc_cisvec(I, i, c, omo, umo, llim, cis_size, cisvecs) * calc_cisvec(J, j, b, omo, umo, llim, cis_size, cisvecs) * get_precalc_ints_sd(c, j, a, b, omo, umo, llim, prec_ints)
		      + calc_cisvec(I, j, c, omo, umo, llim, cis_size, cisvecs) * calc_cisvec(J, j, b, omo, umo, llim, cis_size, cisvecs) * get_precalc_ints_sd(c, i, b, a, omo, umo, llim, prec_ints);
	 }
     }
 }
 for(int b = llim + omo; b < (llim + omo + umo); b++){
    for(int j = llim; j < (llim + omo); j++){
	 for(int k = llim; k < (llim + omo); k++){
	     second += calc_cisvec(I, k, a, omo, umo, llim, cis_size, cisvecs) * calc_cisvec(J, j, b, omo, umo, llim, cis_size, cisvecs) * get_precalc_ints_sd(i, j, b, k, omo, umo, llim, prec_ints)
		       + calc_cisvec(I, k, b, omo, umo, llim, cis_size, cisvecs) * calc_cisvec(J, j, b, omo, umo, llim, cis_size, cisvecs) * get_precalc_ints_sd(i, j, k, a, omo, umo, llim, prec_ints);
	 }
     }
 }
return(first + second);
...
double calc_theta(int I, int J, int a, int i, int omo, int umo, int llim, int cis_size, double* MOens, double* cisens, double* cis_vecs, double* prec_ints){
    double theta = (calc_Y(I, J, a, i, omo, umo, llim, cis_size, cis_vecs, prec_ints) / (MOens[a] - MOens[i] + cisens[J] - cisens[I]));
    return(theta);
}
