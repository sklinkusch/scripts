...
module matrix
  ...
  integer,public :: Nbm1,Nbasis,Nsys,Nsm1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  subroutine writePOP(interaction,time,rhoIN,N)

    implicit none
    logical,intent(in) :: interaction
    integer,intent(in) :: N
    real(kind=8),intent(in) :: time
    complex(kind=8),intent(in) :: rhoIN(0:Nbm1,0:Nbm1,N)
    integer :: m,mp,k,l,j,ipol
    real(kind=4) :: population(0:nbm1,N)
    real(kind=8) :: time_tau,norm(N)
    real(kind=8) :: expectDIP(3),IM_pol(3)
    complex(kind=8) :: rhoS(0:Nbm1,0:Nbm1),dissipation(0:Nbm1,0:Nbm1),tmp(0:Nbm1,0:Nbm1)

! for all the N density matrices one wants to propagate, evaluate the derivative
    do J = 1,N
...
! convert density to Schroedinger representation
        do mp = 0,Nbm1
        do m = 0,Nbm1
          rhoS(m,mp) = exp(-icmplx*dble(trans_energy(m,mp))*time)*rhoIN(m,mp,J)
        enddo
        enddo
      else
        do mp = 0,Nbm1
        do m = 0,Nbm1
          rhoS(m,mp) = rhoIN(m,mp,J)
        enddo
        enddo
      endif
...
! COMPUTE dipole expectation value
      ...
      do m = 0,Nbm1
        do mp = 0,Nbm1
          do ipol = 1,3
            expectDIP(ipol) = expectDIP(ipol) + dipole(ipol,mp,m)*dble(rhoS(mp,m))
          enddo
        enddo
      enddo
!
...
! compute the imaginary part of the polarizability
      ...
      call matvec(time_tau,rhoS,tmp,1)
      IM_pol(:) = zero
      do m = 0,Nbm1
        do mp = 0,Nbm1
          do ipol = 1,3
            IM_pol(ipol) = IM_pol(ipol) + dipole(ipol,mp,m)*tmp(mp,m)
          enddo
        enddo
      enddo
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! output
      write(8,*) j,real(time),expectDIP,real(time_tau),IM_pol

    enddo
    write(7,*) real(time),(evalfield(time,ipol),ipol=1,npol)
    write(9,*) real(time),(population(:,j),j=1,N),' norm, ',norm(:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  ...
  subroutine matvec(T,rhoIN,rhoOUT,N)

    implicit none
    integer,intent(in) :: N
    real(kind=8),intent(in) :: T
    complex(kind=8),intent(in) :: rhoIN(0:Nbm1,0:Nbm1,N)
    complex(kind=8),intent(out) :: rhoOUT(0:Nbm1,0:Nbm1,N)
...
! diagonal elements
        ...
        do i = 0,Nbm1
          do ipol = 1,Npol
            dumc = dumc - two*Et(ipol)*dipole(map_dip(ipol),i,mp)*dimag(rhoIN(i,mp,J))
          enddo
        enddo
        do i = 0,Nbm1
          dumc = dumc + (rates(i,mp)*rhoIN(i,i,J) - rates(mp,i)*rhoIN(mp,mp,J)) 
        enddo
        rhoOUT(mp,mp,J) = dumc + dimag(trans_energy(mp,mp))*rhoIN(mp,mp,J)
! off-diagonal elements
        do m = mp+1,Nbm1
          dumc = zeroc
          do i = 0,Nbm1
            do ipol = 1,Npol
              dumc = dumc + Et(ipol)*(dipole(map_dip(ipol),i,m)*rhoIN(i,mp,J) - dipole(map_dip(ipol),i,mp)*conjg(rhoIN(i,m,J)))
            enddo
          enddo
          dumc = icmplx*(dumc - trans_energy(m,mp)*rhoIN(m,mp,J)) - dephasing(m,mp)*rhoIN(m,mp,J)
          rhoOUT(m,mp,J) = dumc
          rhoOUT(mp,m,J) = conjg(dumc)
        enddo
      enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  ...
  subroutine matvecI(T,rhoIN,rhoOUT,N)
...
! convert density to Schroedinger representation
      do mp = 0,Nbm1
      do m = 0,Nbm1
        rhoS(m,mp) = exp(-icmplx*dble(trans_energy(m,mp))*T)*rhoIN(m,mp,J)
      enddo
      enddo
...
! off-diagonal elements
        do m = mp+1,Nbm1
          ...
          do i = 0,Nbm1
            do ipol = 1,Npol
              dumc = dumc + Et(ipol)*(dipole(map_dip(ipol),i,m)*rhoS(i,mp) - dipole(map_dip(ipol),i,mp)*conjg(rhoS(i,m)))
            enddo
          enddo
          dumc = exp(icmplx*dble(trans_energy(m,mp))*T)*(icmplx*dumc - dephasing(m,mp)*rhoS(m,mp)) &
                    + dimag(trans_energy(m,mp))*rhoIN(m,mp,J)
          rhoOUT(m,mp,J) = dumc
          rhoOUT(mp,m,J) = conjg(dumc)
        enddo
      enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  ...
  subroutine matvecRW(T,rhoIN,rhoOUT,N)
...
! diagonal elements
        ...
        do i = 0,Nbm1
          if(deltaK(i,mp)) then
            do ipol = 1,Npol
              dumc = dumc + Et(ipol)*dimag(RWfield(i,mp)*dipole(map_dip(ipol),i,mp)*rhoIN(i,mp,J))
            enddo
          endif
        enddo
        dumc = -dumc
...
! off-diagonal elements
        do m = mp+1,Nbm1
          ...
          do i = 0,Nbm1
            if(deltaK(m,i)) then
              do ipol = 1,Npol
                dumc = dumc + Et(ipol)*RWfield(m,i)*dipole(map_dip(ipol),i,m)*rhoIN(i,mp,J)
              enddo
            endif
            if(deltaK(i,mp)) then
              do ipol = 1,Npol
                dumc = dumc - Et(ipol)*RWfield(i,mp)*dipole(map_dip(ipol),i,mp)*conjg(rhoIN(i,m,J))
              enddo
            endif
          enddo
          dumc = half*icmplx*dumc - dephasing(m,mp)*rhoIN(m,mp,J) + dimag(trans_energy(m,mp))*rhoIN(m,mp,J)
          rhoOUT(m,mp,J) = dumc
          rhoOUT(mp,m,J) = conjg(dumc)
        enddo
      enddo
