...
//Functions
void cp_mus(int cis_size, int nstates, int nroao, int omo, int umo, int llim,  int nroe, double* cismat, double* cisvecs, double* D,
	    double mu_core, double* MOs, double* Pmat, double* tmpvecs, double* cistmpmat,
	    ofstream *syf, long int &nonzero, int mode, ofstream *outf);
...
outf.flush();
  ...
  outf << "X\n";
  outf.flush();
  cp_mus(cis_size, nstates, nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dx,
	 mu_core[0], MOs, Pmat, tmpmat1, cistmpmat, &syf, nonzero, 2, &outf);
  double* dipx = new double[nstates*nstates];
  for(int x = 0; x < nstates; x++){
      for(int y = 0; y < nstates; y++) dipx[x*nstates+y] = cismat[x*nstates+y];
  }
  diag_mat(nstates, dipx, cisvals, cistmpmat);
  datf.write((char *) cisvals, nstates*sizeof(double));
  datf.write((char *) cistmpmat, nstates*nstates*(long long int) sizeof(double));
  datf.flush();
  outf << "Y\n";
  outf.flush();
  cp_mus(cis_size, nstates, nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dy,
	 mu_core[1], MOs, Pmat, tmpmat1, cistmpmat, &syf, nonzero, 2, &outf);
  double* dipy = new double[nstates*nstates];
  for(int x = 0; x < nstates; x++){
      for(int y = 0; y < nstates; y++) dipy[x*nstates+y] = cismat[x*nstates+y];
  }
  diag_mat(nstates, dipy, cisvals, cistmpmat);
  datf.write((char *) cisvals, nstates*sizeof(double));
  datf.write((char *) cistmpmat, nstates*nstates*(long long int) sizeof(double));
  datf.flush();
  outf << "Z\n";
  outf.flush();
  cp_mus(cis_size, nstates, nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dz,
	 mu_core[2], MOs, Pmat, tmpmat1, cistmpmat, &syf, nonzero, 2, &outf);
  double* dipz = new double[nstates*nstates];
  for(int x = 0; x < nstates; x++){
      for(int y = 0; y < nstates; y++) dipz[x*nstates+y] = cismat[x*nstates+y];
  }
  diag_mat(nstates, dipz, cisvals, cistmpmat);
  datf.write((char *) cisvals, nstates*sizeof(double));
  datf.write((char *) cistmpmat, nstates*nstates*(long long int) sizeof(double));
  datf.flush();
...
//  syf->write((char *) cismat, sizeof(double)*cis_size*cis_size);
  ...
  if(mode == 2){
      calc_mu_mat_cis(cis_size, nroao, omo, umo, llim, nroe, cismat, D, mu_core, MOs, Pmat, tmpvecs, outf);
