...
void cp_mus(int cis_size, int nroao, int omo, int umo, int llim,  int nroe, double* cismat, double* cisvecs, double* D,
	    ...
	    ofstream *syf, ofstream *outf, long int &nonzero, int mode);


//Extern Functions
extern void mat_T_mat(long long int np, double* mat_i1, double* mat_i2, double* mat_f);       //for large matrices
...
extern void renorm_vec(long long int np, double* vecs);
extern void gram_schmidt(long long int nros, double* vecs);
...
#pragma omp parallel for
  ...
  for(long long int x = 2; x < 4; x++){
      for(long long int y = 0; y < Cis_size; y++){
	  printf("%10.6f ", cisvecs[x*Cis_size+y]);
	  if(y%10 == 9) cout << "\n";
      }
      cout << "\n\n";
  }
...
//  renorm_vec(Cis_size, cisvecs);
  gram_schmidt(Cis_size, cisvecs);
  /* // Output for debugging
  for(long long int x = 2; x < 4; x++){
      for(long long int y = 0; y < Cis_size; y++){
	  printf("%10.6f ", cisvecs[x*Cis_size+y]);
  ...
  */
  ...
  datf.flush();
//  double* dummmat = new double[Cis_size*Cis_size];
//  double devmat = 0.;
//  mat_T_mat(Cis_size, cisvecs, cisvecs, dummmat);
//  for(long long int x = 0; x < Cis_size; x++){
//      for(long long int y = 0; y < Cis_size; y++){
//	  if(x == y){
//	      devmat += fabs(dummmat[x*Cis_size+y] - 1.);
//	  }else{
//	      devmat += fabs(dummmat[x*Cis_size+y] - 0.);
//	  }
//      }
//  }
//  devmat / pow((double) Cis_size,2.);
//  cout << "Deviation in matrix: " << devmat << "\n";
...
#pragma omp parallel for
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dx,
	 mu_core[0], MOs, Pmat, tmpmat1, cistmpmat, &syf, &outf, nonzero, 2);
...
#pragma omp parallel for
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dy,
	 mu_core[1], MOs, Pmat, tmpmat1, cistmpmat, &syf, &outf, nonzero, 2);
...
#pragma omp parallel for
  ...
  cp_mus(cis_size,  nroao,  omo,  umo, llim, nroe,  cismat,  cisvecs, Dz,
	 mu_core[2], MOs, Pmat, tmpmat1, cistmpmat, &syf, &outf, nonzero, 2);
