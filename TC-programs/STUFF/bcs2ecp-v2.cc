...
int main(int argc, char* argv[]){
    if(argc != 8 && argc != 9){
	cerr << "Usage: ./bcs2ecp <input ecp file> <input bcs file> <switch_nstates> <offset energy> <maximal energy> <input irx file> <IP> <output prefix>\n";
	cerr << "or: ./bcs2ecp <input bcs file> <switch_nstates> <offset energy> <maximal energy> <input irx file> <IP> <output prefix>\n";
	exit(1);
    }
    if(argc == 9){
	...
	int switch_nstates = atoi(argv[3]);;
	...
	double offset = strtod(argv[4], NULL);                                                                                 // energy offset (added to each electronic state) in Hartree
	double maxen = strtod(argv[5], NULL);                                                                                  // maximal energy (higher states are cut off) in Hartree
	...
	sprintf(finirx, "%s", argv[6]);                                                                                        // store commandline argument 6 in finirx
	...
	sprintf(foutecp, "%s.ecp", argv[8]);                                                                                   // store commandline argument 9 in foutecp
	...
	bcsf.open(finbcs);                                                                                                     // connect filestream bcsf with filename finbcs and open file
	int switch_nstates = atoi(argv[2]);
	int nroao, nroe, llim, ulim, omo, umo, nstates, cis_size;                                                              // define integers (number of atomic orbitals, number of electrons, lower/upper limit, occ./unocc. orb., number of states)
	bcs_size(&bcsf, nroao, nroe, llim, ulim, omo, umo, nstates, cis_size, switch_nstates);                                 // read size parameters nroao, nroe, llim, and ulim from bcs file and calculate omo, umo, and cis_size
