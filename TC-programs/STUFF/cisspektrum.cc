...
int main(int argc, char* argv[]){
  ...
  outf << "Increment: " << incr;
  if(corv == 1){
      outf << "Reading energy corrections from: " << corrfile << "\n";
  }else{
      outf << "Energies are uncorrected\n";
  }
  outf.flush();
  
  double* cens  = new double[nros];
  double* ens   = new double[nros];
  double* corr  = new double[nros];
...
#pragma omp parallel for
  for(int x = 0; x < nros; x++){
   cens[x] = ens[x] + corr[x];                          // correct energies (CIS(D), Grimme-SCIDFT)
