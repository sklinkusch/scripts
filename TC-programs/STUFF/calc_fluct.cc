...
int main(int argc, char* argv[]){
    if(argc != 7){
	cerr << "Need HFdensity file, input-prefix, output-prefix, llim, nroe, ulim!\n";
	exit(0);
    }
// read Hartree-Fock density
    ...
    long double* HFdens = new long double[dim_rd];
    ...
    inf.close();
    for(int j = 0; j < dim_rd; j++) HFdens[j] = dens_rd[j];
	...
	inf.read((char *) dens_rd, sizeof(long double)*dim_rd);
	inf.close();
	for(int j = 0; j < dim_rd; j++) dedens[j] = HFdens[j] - (long double) pow(dens_rd[j],2.);
	for(int v = (homo+1); v <= ulim; v++){
	    ...
	    inf.read((char *) dens_rd, sizeof(long double)*dim_rd);
	    inf.close();
	    for(int j = 0; j < dim_rd; j++) findens[j] = dedens[j] + (long double) pow(dens_rd[j],2.);
