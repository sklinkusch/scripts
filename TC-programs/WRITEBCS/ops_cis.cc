...
void build_cismat(long long int Cis_size, double* cismat, double* cisvals, double* cisvecs);
...
void build_cismat(long long int Cis_size, double* cismat, double* cisvals, double* cisvecs){
    double* tempmat = new double[Cis_size*Cis_size];
    for(long long int x = 0; x < Cis_size; x++){
	for(long long int y = 0; y < Cis_size; y++){
	    if(x == 0 && y == 0){
		tempmat[x*Cis_size+y] = 0.;
		continue;
	    }else if(x == y){
		tempmat[x*Cis_size+y] = cisvals[x];
	    }else{
		tempmat[x*Cis_size+y] = 0.;
	    }
	}
    }
    double* tmpmat = new double[Cis_size*Cis_size];
    mat_mat(Cis_size, cisvecs, tempmat, tmpmat);
    mat_mat_T(Cis_size, tmpmat, cisvecs, cismat);
}
