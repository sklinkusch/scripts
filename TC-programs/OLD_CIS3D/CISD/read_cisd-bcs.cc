...
int main(int argc, char* argv[]){
  ...
  for(int x = 0; x<  nros; x++){
    sprintf(dumc,"State: %.5i   Exc. Energy: %4.14f",x,ens[x]-ens[0]);
    cout << dumc << "\n";
  }
  ...
  datf.read((char *) dvals, cisd_size * sizeof(double));
  datf.read((char *) dmat, cisd_size* cisd_size * sizeof(double));
  for(int x = 0; x < cisd_size; x++) dummat[x*cisd_size+x] = dvals[x];


  //OLD SLOW VERSION  
//   if(cisd_size < 10000)
//     trans_mat((int) cisd_size, dummat, dmat, vecs, 0); //Forward transfornation
//   else
//     trans_mat(cisd_size, dummat, dmat, vecs, 0); //Forward transfornation
  
  for(int x = 0; x<  nros; x++){
    double val = 0.;
    for(int y = 0; y < cisd_size; y++) val += dmat[y*cisd_size+x]*dvals[y]*dmat[y*cisd_size+x];
  ...
  for(int x = 0; x<  nros-1; x++){
	...
	double val = 0.;
	for(int z = 0; z < cisd_size; z++) val += dmat[z*cisd_size+x]*dvals[z]*dmat[z*cisd_size+y];
  ...
  datf.read((char *) dvals, cisd_size * sizeof(double));
  datf.read((char *) dmat, cisd_size* cisd_size * sizeof(double));
  for(int x = 0; x < cisd_size*cisd_size; x++) dummat[x] = 0.;
  for(int x = 0; x < cisd_size; x++) dummat[x*cisd_size+x] = dvals[x];
...
//     trans_mat(cisd_size, dummat, dmat, vecs, 0); //Forward transfornation
  ...
  datf.read((char *) corr, cisd_size * sizeof(double));
