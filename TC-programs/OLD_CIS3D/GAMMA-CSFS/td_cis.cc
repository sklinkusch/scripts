...
extern void transformationCIS_SE(Complex* wav, Complex* SEwav, double* cis_vecs, long long int cis_size);
...
int main(int argc, char* argv[]){
  ...
  char    hfwfile[256];   //read in initial wavefunction
  ...
  ist >> hfwfile >> dt >>  mw_exp >> mw_pop >> mw_wav >> use_d >> damp >> ip >> nrolp; 
  
  outf << "\n Wave function read from " << hfwfile << "\n";
  outf << "\nSystem data \nsys-file: " << bcsfile << "\n\n";
  ...
  outf << "Damping parameter set to:\t" << damp << "\n";
  ...
  double* gamma     = new double[cis_size];        //ionization rate function
  ...
  Complex* PsiC = new Complex[cis_size];      //WF in CSF space
  double* A = new double[cis_size*cis_size]; //Transformation matrix
  

  for(long long int x = 0; x < cis_size; x++){
   for(long long int y = 0; y < cis_size; y++){
   A[x*cis_size+y] = cisvecs[y*cis_size+x];
  }
}
  ...
  //READING IN MO ENERGIES
  double* MOens = new double[cis_size];
  double* MOs = new double[cis_size*cis_size];
   read_wav_HF(hfwfile, nroao, MOens, MOs);
  int MOr = 0;
  for(int x = 0; x < cis_size; x++){
   MOr = (x-1)%umo+omo+llim;
  if((MOens[MOr]-ip) < 0){
    gamma[x] = 0.;
   }
   else{
    gamma[x] = damp*sqrt(MOens[MOr]-ip);
   }
   }
...
//       }
    ...
    //Transform into CSF space
    transformationCIS_SE(PsiE, PsiC, cisvecs, (long long int) cis_size);

    //Insert damped Energies
    for(int x = 0;  x < cis_size; x++)
     PsiC[x] = exp(-1.*gamma[x]*dt)*PsiC[x];

     transformationCIS_SE(PsiC, PsiE, A, (long long int) cis_size);
