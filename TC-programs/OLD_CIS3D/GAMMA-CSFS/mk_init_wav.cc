...
int main(int argc, char* argv[]){
  ...
  if(iwav_mode == 0){
    ...
    //Eigenstate basis
    int i_state;                  //inital state
    ist >> i_state;
    outf << "Initial state is " << i_state << "\n";
    wav[i_state] = 1.; 
  }
  ...
  if(iwav_mode == 1){
    ...
    ist >> i_MO >> f_MO;
    outf << "Inital MO is " << i_MO << "\n";
    outf << "Final  MO is " << f_MO << "\n";
    SEwav[pos_i_a(i_MO, f_MO, omo, umo, llim)] = 1.;
  ...
  if(iwav_mode == 2){
    ...
    outf << "\nNormalizing initial and final orbital\n";
    // hier fängt es an................................
    // Normierungsbed.:
    // \vec C_i^\dagger * \mat S * \vec C_i = 1 
    double mo_norm;

    // dumvec = \mat S * \vec C_i
    pmv(Smat, i_MO , dumvec, nroao);
    mo_norm = 0.;
    
    // \vec C_i^\dagger * dumvec
    for(int x = 0; x < nroao; x++)
      mo_norm += i_MO[x]*dumvec[x];
    outf << "Original norm of inital MO is: " << mo_norm << "\n";
    //renorming
    for(int x = 0; x < nroao; x++) i_MO[x] /= sqrt(mo_norm);
    ...
    // \vec C_i^\dagger * dumvec
    for(int x = 0; x < nroao; x++)
      mo_norm += f_MO[x]*dumvec[x];
    outf << "Original norm of final MO is: " << mo_norm << "\n";
    //renorming
    for(int x = 0; x < nroao; x++) f_MO[x] /= sqrt(mo_norm);
