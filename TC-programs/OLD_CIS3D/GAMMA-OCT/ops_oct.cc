...
//Functions
void single_prop(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
...
void prop_fore(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
...
void prop_fore_mt(Complex* PsiE, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
		  ...
		  int cis_size, int ts, double dt, int nrots, double* field_h, double* field_b, double* gamma);
...
void single_prop(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
		 ...
		 double* cisvecs,  double* A, double* gamma, double* mu_vecs_x,  double* mu_vecs_y, double* mu_vecs_z,
		 int cis_size, int nrots, double dt, double* field_h, char* pref, 
		 int mw_exp, int mw_pop, int mw_wav, ofstream *outf, int extra_ts, int csf, 
                 double* kin, double ekin, double* tmpvec){
  char dumc[256];
  ...
  *outf << "Writing time dependent PsiE to " << dumc << "\n";
  ...
  sprintf(dumc,"%s.kin",pref);
  ofstream kinf(dumc);
  *outf << "Writing time-dependent kinetic energies (of escaping electron) to " << dumc << "\n";
...
//      }
    ...
    if(t%mw_pop == 0){
     ...
     for(int x = 0; x < cis_size; x++){
      for(int y = 0; y < cis_size; y++){
       ekin += sqrt(norm(PsiE[x]))*pow(cisvecs[x*cis_size+y],2)*kin[x];
      }
     }
     kinf << t*dt << " " << ekin << "\n";
     kinf.flush();
    ...
    if(t < nrots)
      prop_fore(PsiE, PsiC, PsiM_x,  PsiM_y,  PsiM_z,
		cis_vals, cisvecs, A, gamma, mu_vals_x,   mu_vals_y,  mu_vals_z,
		mu_vecs_x,   mu_vecs_y,  mu_vecs_z,
		cis_size, t, dt, nrots, field_h, csf, tmpvec, kin, ekin);
    else
      prop_fore(PsiE,  PsiC, PsiM_x,  PsiM_y,  PsiM_z,
		cis_vals, cisvecs, A, gamma, mu_vals_x,   mu_vals_y,  mu_vals_z,
		mu_vecs_x,   mu_vecs_y,  mu_vecs_z,
		cis_size, 0, dt, 0, zero_field, csf, tmpvec, kin, ekin);
  }
}
...
void prop_fore(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
  ...
  if(csf == 1){
   transformationCIS_SE(PsiE, PsiC, cisvecs, Cis_size);
  }else{
   for(int x = 0; x < cis_size; x++){
    PsiC[x] = PsiE[x];
   }
    ...
    //Insert damped energies
    for(int x = 0; x < cis_size; x++){
     PsiC[x] = exp(-1. * gamma[x] * dt) *PsiC[x];
    }

  if(csf == 1){
   transformationCIS_SE(PsiC, PsiE, A, Cis_size);
  }else{
   for(int x = 0; x < cis_size; x++){
    PsiE[x] = PsiC[x];
   }
