...
extern void single_prop(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
			double* cis_vals, double* mu_vals_x,  double* mu_vals_y, double* mu_vals_z,
			double* cisvecs, double* A, double* gamma,  double* mu_vecs_x,  double* mu_vecs_y, double* mu_vecs_z,
			int cis_size, int nrots, double dt, double* field_h, char* pref, 
			int mw_exp, int mw_pop, int mw_wav, ofstream *outf, int extra_ts, int csf, double* kin, double ekin,
                        double* tmpvec);
...
extern void   transformationCIS_SE(Complex* wav, Complex* SEwav, double* cisvecs, long long int cis_size);

extern void prop_fore(Complex* PsiE, Complex* PsiC, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
		      double* cis_vals, double* cisvecs, double* A, double* gamma, double* mu_vals_x,  double* mu_vals_y, double* mu_vals_z,
		      double* mu_vecs_x,  double* mu_vecs_y, double* mu_vecs_z,
		      int cis_size, int ts, double dt, int nrots, double* field_h, int csf, double* tmpvec, double* kin, double ekin);
...
int main(int argc, char* argv[]){
 ...
 //SYSTEM
  ...
  char    hfwfile[256];   //wave function
  char    opfile[256];    //operator or target wave function file
  char    effile[256];    //file containing electric field
  char    gamfile[256];   //file containing gamma values
  ...
  double  ip;             //vacuum energy
  int     csf;            //method for gammas to be calculated
  int     cal_gam;        //0: without gammas; 1: calculate gammas; 2: read in gamma values
  ...
  ist >> hfwfile;
  outf << "MO energies are read from " << hfwfile << "\n"; 
  ...
  ist >> cal_gam;
   if(cal_gam == 2){
    ist >> gamfile;
    outf << "Reading gamma values from: " << gamfile << "\n";
   }else if(cal_gam == 1){
    outf << "Calculating gamma values\n";
   }else{
    outf << "Using non-ionizing system\n";
   }

  ist >> damp;
  outf << "Damping coefficient set to: " << "\t" << damp << "\n";

  ist >> ip;
  outf << "Vacuum energy set to: " << "\t" << ip << "\n";

  ist >> csf;
  if(csf == 1){
   outf << "MO energies are damped\n";
  }else{
   outf << "Eigenenergies are damped\n";
  }
  ...
  double* MOens     = new double[cis_size];        //MO energies
  double* MOs       = new double[cis_size*cis_size];   //Molecular orbitals
  double* gamma     = new double[cis_size];        //Gamma value
  double* tmpvec    = new double[2*cis_size];
  double* kin       = new double[cis_size];        // kinetic energy of escaping electron
  double  ekin;

  double* cisvecs   = new double[(long long int) cis_size * (long long int) cis_size];        //eigenvectros of CI-matrix
  double* A         = new double[(long long int) cis_size * (long long int) cis_size];        //transpose of cisvecs
  ...
  Complex* PsiE;                              //Wave function in energy space
  Complex* PsiC = new Complex[cis_size];      //Wave function in CSF space
  ...
  Complex* ChiE;                              //Wave function in energy space
  Complex* ChiC = new Complex[cis_size];      //Dummy variable
  ...
  for(int x = 0; x < cis_size; x++){
   ...
   for(int y = 0; y < cis_size; y++){
    A[y*cis_size+x] = cisvecs[x*cis_size+y];
    }
   }
  ...
  // READING IN MO ENERGIES, CALCULATING GAMMA
...
if(cal_gam == 1){
  ...
  int MO_r = 0;
   for(int x = 0; x < cis_size; x++){
    ...
    if((MOens[MO_r]-ip) < 0){
     ...
     gamma[x] = damp*sqrt(MOens[MO_r]-ip);
    }
   }
  }else if(csf == 2){
   for(int x = 0; x < cis_size; x++){
    if((cis_vals[x] - ip) < 0){
     ...
     int MO_r = 0; 
     for(int y = 0; y < cis_size; y++){
      MO_r = (y-1)%umo+omo+llim;
      if(MOens[MO_r] > 0){
      kin[x] += pow(fabs(cisvecs[x*cis_size+y]),2)*MOens[MO_r];
      gamma[x] += pow(fabs(cisvecs[x*cis_size+y]),2)*damp*sqrt(MOens[MO_r]);
      }else{
      gamma[x] += 0.;
      kin[x] += 0.;
      }
   ...
   for(int x = 0; x < cis_size; x++){
    if((cis_vals[x]-ip) < 0){
     ...
     gamma[x] = damp*sqrt(cis_vals[x]-ip);
    }
   }
  }
}else if(cal_gam == 2){
 ...
 for(int x = 0; x < cis_size; x++){
  ...
  //WRITING OUT GAMMA VALUES
  outf << "-------------------------------------------------------------------------------\n";
  outf << "Gamma Values: \n";
  for(int x = 0; x < cis_size; x++){
  outf << "Initial State: " << x << "\t" << gamma[x] << "\n"; 
  }
  outf << "*******************************************************************************\n";
  ...
  if(nroit < 1)
    single_prop( PsiE, PsiC,  PsiM_x,  PsiM_y,  PsiM_z,
		 cis_vals,  mu_vals_x,   mu_vals_y,  mu_vals_z,
		 cisvecs,  A, gamma,  mu_vecs_x,   mu_vecs_y,  mu_vecs_z,
		 cis_size,  nrots,  dt,  field_h, dumc, 
		 mw_exp,  mw_pop,  mw_wav, &outf, extra_ts, csf, kin, ekin, tmpvec);
  else
    single_prop( PsiE,  PsiC, PsiM_x,  PsiM_y,  PsiM_z,
		 cis_vals,  mu_vals_x,   mu_vals_y,  mu_vals_z,
		 cisvecs,  A, gamma,  mu_vecs_x,   mu_vecs_y,  mu_vecs_z,
		 cis_size,  nrots,  dt,  field_h, dumc, 
		 mw_exp,  mw_pop,  mw_wav, &outf, 0, csf, kin, ekin, tmpvec);
...
//    }
	...
	prop_fore(PsiE,  PsiC, PsiM_x,  PsiM_y,  PsiM_z, cis_vals, cisvecs, A, gamma, mu_vals_x,  mu_vals_y, mu_vals_z, mu_vecs_x, mu_vecs_y, mu_vecs_z, cis_size, x, -dt, nrots, field_h, csf, tmpvec, kin, ekin);
	prop_fore(ChiE,  ChiC, ChiM_x,  ChiM_y,  ChiM_z, cis_vals, cisvecs, A, gamma, mu_vals_x,  mu_vals_y, mu_vals_z, mu_vecs_x, mu_vecs_y, mu_vecs_z, cis_size, x, -dt, nrots, field_b, csf, tmpvec, kin, ekin);
      }else{
	//Backward mt (-dt)
	prop_fore_mt(PsiE, PsiM_x, PsiM_y, PsiM_z,   cis_vals,   mu_vals_x,   mu_vals_y,  mu_vals_z,    mu_vecs_x,   mu_vecs_y,   mu_vecs_z,
		     ...
		     cis_size, x, -dt, nrots,  field_h, field_b, gamma);
      }
