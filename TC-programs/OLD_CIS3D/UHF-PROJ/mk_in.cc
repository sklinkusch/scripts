...
//EXTERN FUNCTIONS
extern "C" void dafrd_(double* V, int* LEN, int* RECN, char* FNAME);
...
void read_integrals_12(void){
  ...
  for(int x = 0; x < nrorec; x++){
    inf.read(dumchar, 12);   //8 bytes junk at start 
...
void read_integrals_16(void){
  ...
  for(int x = 0; x < nrorec; x++){
    inf.read(dumchar, 8);   //8 bytes junk at start 
