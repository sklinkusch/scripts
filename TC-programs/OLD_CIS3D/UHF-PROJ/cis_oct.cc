...
extern void single_prop(Complex* PsiE, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
			...
			int mw_exp, int mw_pop, int mw_wav, ofstream *outf, int extra_ts);
...
extern void prop_fore(Complex* PsiE, Complex* PsiM_x, Complex* PsiM_y, Complex* PsiM_z,
		      ...
		      int cis_size, int ts, double dt, int nrots, double* field_h);
 ...
 //SYSTEM
  ...
  if(nroit < 1)
    single_prop( PsiE,  PsiM_x,  PsiM_y,  PsiM_z,
		 ...
		 mw_exp,  mw_pop,  mw_wav, &outf, extra_ts);
  else
    single_prop( PsiE,  PsiM_x,  PsiM_y,  PsiM_z,
		 cis_vals,  mu_vals_x,   mu_vals_y,  mu_vals_z,
		 cisvecs,   mu_vecs_x,   mu_vecs_y,  mu_vecs_z,
		 cis_size,  nrots,  dt,  field_h, dumc, 
		 mw_exp,  mw_pop,  mw_wav, &outf, 0);
  ...
  for(int cit = 1; cit <= nroit; cit++){
	...
	prop_fore(PsiE,  PsiM_x,  PsiM_y,  PsiM_z, cis_vals,  mu_vals_x,   mu_vals_y,  mu_vals_z, mu_vecs_x,   mu_vecs_y,  mu_vecs_z,cis_size, x, -dt, nrots, field_h);
	prop_fore(ChiE,  ChiM_x,  ChiM_y,  ChiM_z, cis_vals,  mu_vals_x,   mu_vals_y,  mu_vals_z, mu_vecs_x,   mu_vecs_y,  mu_vecs_z,cis_size, x, -dt, nrots, field_b);
      }else{
	//Backward mt (-dt)
