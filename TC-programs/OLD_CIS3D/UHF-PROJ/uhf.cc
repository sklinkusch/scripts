...
extern void build_Fmat_uhf(int nroao, double* Fmat, double* Pmat, double* Pcmat, double* Hmat,
		       double* intval, unsigned short* intnums,
		       long long int* sortcount, long long int nrofint);
extern double Calc_e_el(int nroao, double* Famat, double* Fbmat, double* Pmat, double* Pamat, double* Pbmat, double* Hmat);
...
int main(int argc, char* argv[]){
  ...
  int     nroae;          //Nr of alpha electrons
  int     nrobe;          //Nr of beta electrons
...
*/
  ...
  outf << "System data \nNr of electrons: " << nroe << "\nNr of alpha electrons: " << nroae << "\nNr of beta electrons: " << nrobe << "\nMultiplicity: " << mult << "\nsys-file: " << sysfile << "\n\n";
  ...
  double*        Pamat;         //density matrix (alpha)            nroao*nroao
  double*        Pbmat;         //density matrix (beta)             nroao*nroao
  double*        Pmat_old;      //Old density matrix                nroao*nroao
  double*        Pamat_old;     //Old density matrix (alpha)        nroao*nroao
  double*        Pbmat_old;     //Old density matrix (beta)         nroao*nroao

  double*        Fmat;          //Fock matrix                       nroao*nroao
  double*        Famat;         //Fock matrix (alpha)               nroao*nroao
  double*        Fbmat;         //Fock matrix (beta)                nroao*nroao
  ...
  build_Pmat_dscf(nroao, nroae, Pamat, Pamat_old, MOs, 0.);
  build_Pmat_dscf(nroao, nrobe, Pbmat, Pbmat_old, MOs, 0.);
  for(int i = 0; i < (nroao*nroao); i++){
   Pmat[i]=Pamat[i]+Pbmat[i];
  }
  build_Fmat_uhf(nroao,  Famat, Pmat, Pamat, Hmat, intval, intnums, sortcount, nrofint);
  build_Fmat_uhf(nroao,  Fbmat, Pmat, Pbmat, Hmat, intval, intnums, sortcount, nrofint);


  outf << "Guess Energy: " << Calc_e_el( nroao, Famat, Fbmat, Pmat, Pamat, Pbmat, Hmat) + ion_rep << "\n";
  outf << "-------------------------------------------------------------------------------\n";
  outf << "Entering SCF:\n";
  ...
  double maxd_densa, maxd_densb;
  
  oldE = Calc_e_el( nroao, Famat, Fbmat, Pmat, Pamat, Pbmat, Hmat);
  ...
  for(int c = 0; c < max_cyc; c++){
    ...
    diag_Fmat( nroao, Famat, MOs,  MOens,  Som12, tmpmat1);
    diag_Fmat( nroao, Fbmat, MOs,  MOens,  Som12, tmpmat1);
    maxd_densa = build_Pmat_dscf( nroao, nroae,  Pamat,  Pamat_old,  MOs, damp);
    maxd_densb = build_Pmat_dscf( nroao, nrobe,  Pbmat,  Pbmat_old,  MOs, damp);
    for(int i = 0; i < (nroao*nroao); i++){
     Pmat[i] = Pamat[i] + Pbmat[i];
    }
    maxd_dens = maxd_densa + maxd_densb;
    build_Fmat_uhf(nroao,  Famat, Pmat, Pamat, Hmat, intval, intnums, sortcount, nrofint);
    build_Fmat_uhf(nroao,  Fbmat, Pmat, Pbmat, Hmat, intval, intnums, sortcount, nrofint);

    currE =  Calc_e_el( nroao, Famat, Fbmat, Pmat, Pamat, Pbmat, Hmat);
    dE    =  currE-oldE;
