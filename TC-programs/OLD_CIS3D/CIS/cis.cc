...
int main(int argc, char* argv[]){
  ...
  if(MOens[0] == 0.){
    outf << "Recalculating MO energies\n";
    for(int x = 0; x < nroao; x++){
      pmv(Fmat, &(MOs[x*nroao]), &(tmpvecs[x*nroao]), nroao);
      MOens[x] = 0.;
      for(int z = 0; z < nroao; z++) MOens[x] += MOs[x*nroao+z]*tmpvecs[x*nroao+z];
    } 
  }
...
char transfile[512];
char dumline[2048];
sprintf(transfile, "%s.trf", argv[2]);
trfs.open(transfile);
for(int x = 1; x < cis_size; x++){
    int mi, mf;
    for(int y = 1; y < cis_size; y++){
        mi = (y-1)/umo+llim;
	mf = (y-1)%umo+omo+llim;
        sprintf(dumline, "%5.d   %4.d   %4.d   % 11.8f", x, mi, mf, cisvecs[x*cis_size+y]);
	trfs << dumline << "\n";
    }
}
trfs.close();
