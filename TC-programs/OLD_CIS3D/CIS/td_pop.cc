...
extern void   calc_dens(int nroe, int nroao, int llim, int ulim, Complex* SEwav, Complex* Pmat, double* MOs);
extern void   mat_mat(int np, Complex* mat_i1, double* mat_i2, Complex* mat_f);
...
extern void   calc_dens_MO(int nroe, int nroao, int llim, int ulim, Complex* SEwav, Complex* Pmat);
extern void   diag_matH(int nroao, Complex* mat, double* vals, Complex* vecs);
int main(int argc, char* argv[]){
  ...
  int     mw_nmo;               //print out of natrual MOs and pops (molden format)
  ...
  double*        Sop12;         //S^+1/2                            nroao*nroao
  ...
  MOs = &(dumd[inc]);  inc += nroao*nroao; Dx =       &(dumd[inc]);   inc += nroao*nroao;  Sop12 =&(dumd[inc]); inc += nroao*nroao; 
  ...
  Complex*        Pmat_Mul;      //Mulliken matrix                   nroao*nroao

  Complex*        Cmat1;         //tmpmat                            nroao*nroao
  Complex*        Cmat2;         //tmpmat                            nroao*nroao
  Complex*        Cmat3;         //tmpmat                            nroao*nroao
  Complex*        Cmat4;         //tmpmat                            nroao*nroao
  
  int ao_cmem = 6*nroao*nroao;
  Complex* dumC = new Complex[ao_cmem]; inc =0;
  Pmat = &(dumC[inc]); inc += nroao*nroao;  Pmat_Mul = &(dumC[inc]); inc += nroao*nroao;

  Cmat1 = &(dumC[inc]); inc += nroao*nroao; Cmat2 = &(dumC[inc]); inc += nroao*nroao;
  Cmat3 = &(dumC[inc]); inc += nroao*nroao; Cmat4 = &(dumC[inc]); inc += nroao*nroao;
  ...
  outf << "\nCalculating S^+1/2\n";
  outf << "Minimal eigenvalue of S is " << calc_Sp12( nroao, Smat, Sop12, tmpmat1, tmpvecs, tmpvals) <<"\n";
  outf << "...............................................................................\n";

  
  ist  >>  nrots >> main_mw >> mw_exp >> mw_mopop >> mw_mat >> mw_nmo;
  ...
  outf << "Natural MOs and Pops:            " << mw_nmo  << "\n";
  ...
  outf << "Kinetic energy: " << calc_one_p_op(nroao, Pmat, Tmat) << "\n";
  outf << "Dipole moment (x,y,z) : " 
       << real(-calc_one_p_op(nroao,   Pmat, Dx)) +mu_core[0] << " "
       << real(-calc_one_p_op(nroao,   Pmat, Dy)) +mu_core[1] << " "
       << real(-calc_one_p_op(nroao,   Pmat, Dz)) +mu_core[2] << "\n";
  ...
  for(int b = 0; b < nrob; b++){
	...
	MayerB += real(conj(Pmat_Mul[c1*nroao+c2])*Pmat_Mul[c2*nroao+c1]);
      }
    }
...
//   Complex* Pmat_MO = new Complex[nroao*nroao];
//   Complex* vecP_MO = new Complex[nroao*nroao];
  ...
  for(int x = 0; x < nrots; x++){
	...
	for(int x = 0; x < cis_size; x++)
	  E+= norm(wav[x])*cis_vals[x];
	expf << curr_time << " " << E << " "
	     << real( calc_one_p_op(nroao, Pmat, Tmat)) << " "
	     << real(-calc_one_p_op(nroao,   Pmat, Dx)) +mu_core[0] << " "
	     << real(-calc_one_p_op(nroao,   Pmat, Dy)) +mu_core[1] << " "
	     << real(-calc_one_p_op(nroao,   Pmat, Dz)) +mu_core[2] << " ";
	//Calc Mulliken matrix
...
//       //STILL BUGGY
      ...
      if((x/main_mw)%mw_nmo == 0 ){
 	if((x/main_mw)%mw_exp != 0){
 	  transformationCIS_SE(wav, SEwav, cisvecs, cis_size);
 	  calc_dens(nroe, nroao, llim, ulim, SEwav, Pmat, MOs);
 	}
	
 	//symortho_mat(nroao, Pmat,  Sop12, Cmat1);

// // 	diag_mat( nroao, Pmat, tmpvals, tmpvecs);
// // 	transform_MOs(nroao, tmpvecs, Som12, tmpmat1);
	
// // 	char fn[256];
// // 	char dc[256];
// // 	sprintf(fn,"%s-%.6i-nmo-mol.dat",argv[2],(x/main_mw)/mw_nmo);
// // 	ofstream nmof(fn);
// // 	nmof << "[MO]\n";
// // 	for(int x = nroao-1; x >= 0; x--){
// // 	  nmof << " Ene=     0.0000\n Spin= Alpha\n Occup=   ";
// // 	  if(tmpvals[x] > 1.e-6)
// // 	    sprintf(dc,"%'.6f",tmpvals[x]);
// // 	  else
// // 	    sprintf(dc,"%'.6f",0.0);
// // 	  nmof << dc << "\n";
// // 	  clog << tmpvals[x] << " ";
// // 	  for(int y = 0; y < nroao; y++){
// // 	    sprintf(dc,"  %i   %'.6f\n", y+1, tmpvecs[x*nroao+y]);
// // 	    nmof << dc;
// // 	  }
// // 	  clog << "\n";
// // 	}
...
// //       diag_mat( nroao, Pmat_MO, tmpvals, tmpvecs);
// //       char fn[256];
// //       char dc[256];
// //       sprintf(fn,"%s-%.6i-nmo-mol.dat",argv[2],(x/main_mw)/mw_nmo);
// //       ofstream nmof(fn);
// //       nmof << "[MO]\n";
// //       for(int i = nroao-1; i >= 0; i--){
// // 	nmof << " Ene=     0.0000\n Spin= Alpha\n Occup=   ";
// // 	if(tmpvals[i] > 1.e-6)
// // 	  sprintf(dc,"%'.6f",tmpvals[i]);
// // 	else
// // 	  sprintf(dc,"%'.6f",0.0);
// // 	nmof << dc << "\n";
// // 	clog << tmpvals[i] << " ";
...
// 	//DEBUG 2  
	double  n = 0.;
	for(int x = 0; x < cis_size; x++) n += norm(SEwav[x]);
	clog << n << "\n";

 	calc_dens_MO(nroe, nroao, llim, ulim, SEwav, Pmat);
	n = 0.; for(int x = 0; x < nroao; x++) n += real(Pmat[x*nroao+x]); 
	clog << n << "\n";
 	diag_matH(nroao, Pmat, tmpvals, Cmat2 );
	double mi = 0.;
	for(int x = 0; x < nroao*nroao; x++){ if(fabs(imag(Cmat2[x])) > mi) mi = fabs(imag(Cmat2[x]));};
	clog << mi <<"\n";
	n = 0.; for(int x = 0; x < nroao; x++) n += tmpvals[x]; 
	clog << n << "\n";
	
 	//for(int i = nroao-1; i >= 0; i--){
	clog << tmpvals[nroao-1] << " ";
	if(fabs(tmpvals[0]) < 1e-12) clog << "0.0" ;
	else clog << tmpvals[0] << "\n";
	  //}
 	clog << "\n\n";
