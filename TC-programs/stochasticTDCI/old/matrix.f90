...
module matrix
  ...
  real(kind=8),allocatable :: dipole(:,:,:),rates(:,:),dephasing(:,:),chi(:)  ! the dipole and the transition rates matrix
  ...
  public matvecI,normalize,initialize,writePOP

contains
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  subroutine initialize(restart,Frst,polarization,N_traj,psiINIT,temperature,&
          pref_ion,pref_diss,pref_deph)

    implicit none
    logical :: restart
    character(len=3),intent(in) :: polarization
    character(len=30),intent(in) :: Frst
    integer,intent(in) :: N_traj
    real(kind=8),intent(in) :: Temperature, pref_ion, pref_diss,pref_deph
...
! read the energies of the configurations, as well as their ionization rates
    ...
    read(2) ionization
    do i=0,Nbm1
       ionization(i) = ionization(i) * pref_ion
    enddo
...
! allocate and initialize
    allocate(dipole(0:Nbm1,0:Nbm1,Npol),rates(0:Nbm1,0:Nbm1),dephasing(0:Nbm1,0:Nbm1),chi(0:nbm1),Et(npol))
    psiINIT(:,:) = zero

! store the components of dipole: use the rate matrix as a temporary array
    ipol = 0
    dipole(:,:,:) = zero
! read x component
    read(2) rates
...
! read relaxation and dephasing rates
    ...
    do i=0,Nbm1
      do j=0,Nbm1
        dephasing(i,j) = dephasing(i,j) * pref_deph
      enddo
    enddo
...
! generate starting density matrix
    if(restart) then
! restart file
      open(8,file=Frst,status='old',form='unformatted',access='stream',iostat=io)
...
! coupling to the electric field
      ...
      do ipol = 1,Npol
        do m = 0,nbm1
          dumc = zeroc
          do mp = 0,nbm1
            dumc = dumc + dipole(mp,m,ipol)*psiS(mp)
          enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  subroutine writePOP(time,ccnt,psi,err,N_traj,Nwrite)

    implicit none
    integer,intent(in) :: N_traj,Nwrite,ccnt 
...
! write coefficients to binary files foutnul (14) and foutext (13)
! sk 2016.01.12
    if(modulo(ccnt,2) .EQ. 0) then
        write(14) time
        do j=1,N_traj
           do i=0,Nbm1
             write(14) psi(i,j)
