...
program stoch_prop
  ...
  implicit none
  logical :: restart
  integer :: i,io,N_traj,Nsteps,Nskip,Nwrite,Npsi,Npsipo
  character(len=3) :: polarization
  character(len=30) :: Fin,Frst,Fenergies,Ffield,method
  character(len=30) :: fout,foutpop,foutavg,foutlog,foutrst,foutsts
  character(len=30) :: foutext,foutnul
  real(kind=8) :: temperature
  real(kind=8) :: eps, pref_ion, pref_diss, pref_deph
...
! name of the output files
  ...
  foutext = trim(fout)//'.ext'
  foutnul = trim(fout)//'.nul'
...
! initialize the matrix: configurations, eigenstates, transition dipoles, rates
  ...
  call initialize(restart,Frst,polarization,N_traj,psi,temperature,pref_ion,&
      pref_diss,pref_deph)
  close(2)
  close(3)
...
! main propagation loop
  ...
  open(13,file=foutext,status='replace',access='stream',form='unformatted')
  open(14,file=foutnul,status='replace',access='stream',form='unformatted')
  write(13) Nbasis
  write(13) N_traj
  write(14) Nbasis
  write(14) N_traj
