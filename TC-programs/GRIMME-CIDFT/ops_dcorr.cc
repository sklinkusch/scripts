...
double RO_calc_v_ia(int i, int a, int omo, int umo, int llim, double* cis_vec, double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows, double* MOens);
double RO_calc_u_term(int i, int j, int a, int b,  int omo, int umo, int llim, double* cis_vec, double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows);
double RO_calc_t1(int omo, int umo, int llim, double* cis_vec, double cis_en, double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows, double* MOens);
double RO_calc_MP2(int omo, int umo, int llim, double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows, double* MOens);
  ...
  for(int k = llim; k < (llim+omo); k++){
   for(int b =  (llim+omo); b < (llim+omo+umo); b++){
    for(int c =  (llim+omo); c < (llim+omo+umo); c++){
     ...
     double JKBC =  get_precalc_ints_ovov(j, b, k, c, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<JK|BC> = (JB|KC) => (JB|KC) -> J<B;K<C
     double JKCB =  get_precalc_ints_ovov(j, c, k, b, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<JK|CB> = (JC|KB) => (JC|KB) -> J<C;K<B

     double CAJK =  get_precalc_ints_ovov(j, c, k, a, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<CA|JK> = (CJ|AK) => (JC|KA) -> J<C;K<A
     double CAKJ =  get_precalc_ints_ovov(k, c, j, a, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<CA|KJ> = (CK|AJ) => (KC|JA) -> K<C;J<A

     double CBIK =  get_precalc_ints_ovov(i, c, k, b, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<CB|IK> = (CI|BK) => (IC|KB) -> I<C;K<B
     double CBKI =  get_precalc_ints_ovov(k, c, i, b, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<CB|KI> = (CK|BI) => (KC|IB) -> K<C;I<B

     double ACIK =  get_precalc_ints_ovov(i, a, k, c, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AC|IK> = (AI|CK) => (IA|KC) -> I<A;K<C
     double ACKI =  get_precalc_ints_ovov(k, a, i, c, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AC|KI> = (AK|CI) => (KA|IC) -> K<A;I<C
...
double RO_calc_u_term(int i, int j, int a, int b,  int omo, int umo, int llim,
                      double* cis_vec, double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows){
 ...
 // get precalculated integrals 
  double ABIC = get_precalc_ints_sd(a, i, b, c, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AB|IC> = (AI|BC)
  double ABCI = get_precalc_ints_sd(a, c, b, i, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AB|CI> = (AC|BI)
  double ABJC = get_precalc_ints_sd(a, j, b, c, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AB|JC> = (AJ|BC)
  double ABCJ = get_precalc_ints_sd(a, c, b, j, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<AB|CJ> = (AC|BJ)
 ...
 for(int k = llim; k < (llim+omo); k++){
  ...
  // get precalculated integrals
  double KAIJ = get_precalc_ints_sd(k, i, a, j, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<KA|IJ> = (KI|AJ)
  double KAJI = get_precalc_ints_sd(k, j, a, i, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<KA|JI> = (KJ|AI)
  double KBIJ = get_precalc_ints_sd(k, i, b, j, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<KB|IJ> = (KI|BJ)
  double KBJI = get_precalc_ints_sd(k, j, b, i, omo, umo, llim, prec_vals, prec_cols, prec_rows);     //<KB|JI> = (KJ|BI)
...
double RO_calc_t1(int omo, int umo, int llim, double* cis_vec, double cis_en,
	                  double* prec_vals, uint32_t* prec_cols, uint32_t* prec_rows, double* MOens){
 double t1 = 0.;
 //MAKE IT FASTER
 //i,i a,a terms unique
 for(int i = llim; i < (llim+omo); i++){
  for(int a =  (llim+omo); a < (llim+omo+umo); a++){
    t1 += 0.25*RO_calc_u_term( i, i, a,  a,  omo, umo, llim, cis_vec, prec_vals, prec_cols, prec_rows)/
 ...
 for(int i = llim; i < (llim+omo); i++){
  for(int a =  (llim+omo); a < (llim+omo+umo); a++){
   for(int b =  a+1; b < (llim+omo+umo); b++){
    t1 += 0.5*RO_calc_u_term( i, i, a,  b,  omo, umo, llim, cis_vec, prec_vals, prec_cols, prec_rows)/
 ...
 for(int i = llim; i < (llim+omo); i++){
  for(int j = i+1; j < (llim+omo); j++){
   for(int a =  (llim+omo); a < (llim+omo+umo); a++){
    t1 += 0.5*RO_calc_u_term( i, j, a,  a,  omo, umo, llim, cis_vec, prec_vals, prec_cols, prec_rows)/
 ...
 for(int i = llim; i < (llim+omo); i++){
  for(int j = i+1; j < (llim+omo); j++){
   for(int a =  (llim+omo); a < (llim+omo+umo); a++){
    for(int b =  a+1; b < (llim+omo+umo); b++){
     t1 +=   RO_calc_u_term( i, j, a,  b,  omo, umo, llim, cis_vec, prec_vals, prec_cols, prec_rows)/
  ...
  for(int j = llim; j < (llim+omo); j++){
   for(int a = (omo+llim); a < (llim+omo+umo); a++){
    for(int b = (omo+llim); b < (llim+omo+umo); b++){
     double J = get_precalc_ints_sd(i, a, j, b, omo, umo, llim, prec_vals, prec_cols, prec_rows);
     double K = get_precalc_ints_sd(i, b, j, a, omo, umo, llim, prec_vals, prec_cols, prec_rows);
     E2 += -0.25/RO_calc_delta_ij_ab( i,  j,  a,  b, MOens)*2.*(pow(J-K,2)+J*J+K*K);
    }
   }
  }
 }
